...
describe("GET /patients/search", () => {
    it("should find patient by patientId", async () => {
        ...
        expect(res.statusCode).toBe(200);
        expect(res.body.results[0].name).toBe("Test Patient");
    });
    ...
    it("should find patient by phone", async () => {
        ...
        expect(res.statusCode).toBe(200);
        expect(res.body.results[0].phone).toBe("8888800001");
    });

    it("should find patient by name (exact case)", async () => {
        const res = await request(app)
            .get("/patients/search")
            .query({ query: "Test Patient" })
            .set("Authorization", `Bearer ${doctorToken}`);

        console.log("SEARCH RESULTS:", JSON.stringify(res.body, null, 2));

        expect(res.statusCode).toBe(200);
        expect(res.body.results.length).toBeGreaterThan(0);
        
        const testPatient = res.body.results.find(r => r.name === "Test Patient");
        expect(testPatient).toBeDefined();
        expect(testPatient.patientId).toBe("PAT_TEST_001");
        expect(testPatient).toHaveProperty("clinicId");
    });

    it("should find patient by name (partial and lowercase)", async () => {
        const res = await request(app)
            .get("/patients/search")
            .query({ query: "test patient" })
            .set("Authorization", `Bearer ${doctorToken}`);

...
export default function VisitWorkflow({ token }: VisitWorkflowProps) {
  ...
  const [, setVisitType] = useState<"NEW" | "FOLLOW_UP">("NEW");
  ...
  const fetchVisit = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const res = await fetch(`http://127.0.0.1:4000/visits/${visitId}`, {
  ...
  async function updateVisitType(type: "NEW" | "FOLLOW_UP") {
    setVisitType(type);
    await fetch(`http://127.0.0.1:4000/visits/${visit.visitId}/type`, {
      method: "PATCH",
  ...
  async function closeVisit() {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:4000/visits/close/${visit.visitId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ 
          isCompleted: closeStatus,
          patientEmail: normalizedEmail || null,
          sendEmail: Boolean(normalizedEmail),
      ...
      let alertMsg = "";
      if (data?.emailError && data?.whatsappError) {
        alertMsg = `Visit closed, but both Email and WhatsApp reports failed to send.\nEmail: ${data.emailError}\nWhatsApp: ${data.whatsappError}`;
      } else if (data?.emailError) {
        alertMsg = `Visit closed, but report Email failed to send: ${data.emailError}`;
      } else if (data?.whatsappError) {
        alertMsg = `Visit closed, but report WhatsApp failed to send: ${data.whatsappError}`;
      }
  ...
  async function handleMedicationsFinish(prescription: any[]) {
    ...
    try {
      await fetch(`http://127.0.0.1:4000/visits/update/${visit.visitId}`, {
        method: "PUT",
        headers: {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <button
          ...
          onClick={async () => {
            try {
              if (visit?.visitId) {
                await fetch(`http://127.0.0.1:4000/visits/${visit.visitId}`, {
                  method: "DELETE",
                  headers: { Authorization: `Bearer ${token}` },
                });
              }
        ...
        {activeStep === 4 && (
          <div className="bg-white rounded-lg border p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:bg-gray-100 disabled:text-gray-400"
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {closing ? (
                  sendWhatsApp && patientPhone.trim() && patientEmail.trim() ? "Closing, Emailing & Sending WhatsApp..." :
                  sendWhatsApp && patientPhone.trim() ? "Closing & Sending WhatsApp..." :
                  patientEmail.trim() ? "Closing & Emailing..." : "Closing Case..."
                ) : "Confirm & Close Visit"}
              </button>
            </div>
          </div>

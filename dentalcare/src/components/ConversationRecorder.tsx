...
export default function ConversationRecorder({ patientId, onDataExtracted, onSkip }: Props) {
  ...
  async function ensureMicrophonePermission() {
    if (window.ipcRenderer?.invoke) {
      const result = await window.ipcRenderer.invoke("media:request-microphone-access") as MicrophonePermissionResult;
      if (!result.granted) {
        throw new Error(result.message || "Microphone permission is required to start recording.");
      }
      return;
    }

    // Browser/dev fallback: trigger prompt and immediately release stream.

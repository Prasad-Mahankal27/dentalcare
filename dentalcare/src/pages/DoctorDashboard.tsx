...
export default function DoctorDashboard({ user }: DoctorDashboardProps) {
  ...
  async function searchPatient(rawQuery?: string | any) {
    const targetQuery = typeof rawQuery === "string" ? rawQuery : query;
    const result = await validatePatientSearch(targetQuery, token);
    if (!result.found) {
  ...
  // Subscribe to appointment changes to refresh stats in real-time
  ...
  useEffect(() => {
    ...
    if (queryFromUrl) {
      ...
      void searchPatient(queryFromUrl);
    }
  ...
  async function startNewVisit() {
    ...
    try {
      ...
      const res = await fetch(
        "http://127.0.0.1:4000/visits/create",
        {
          method: "POST",
          headers: {
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header
          ...
          onLogout={logout}
          rightContent={<SyncIndicator token={token} />}
        />
          ...
          {patient ? (
            ...
            <>
              ...
              <div className="grid lg:grid-cols-2 gap-3 mb-3">
                ...
                <CommonDiseasesReport data={stats.dentalIssuesData} categories={stats.dentalIssueCategories} />
              </div>

...
export default function PatientVisitsPage({ token }: Props) {
    ...
    async function fetchData() {
      try {
        ...
        const patientRes = await fetch(
          `http://127.0.0.1:4000/patients/search?query=${encodeURIComponent(patientId || "")}`,
        ...
        const patientRecord = Array.isArray(patientPayload?.results)
          ? patientPayload.results[0]
          : patientPayload;
        ...
        const visitsRes = await fetch(
          `http://127.0.0.1:4000/visits/history/${patientRecord.patientId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  async function startNewVisit() {
    ...
    const res = await fetch("http://127.0.0.1:4000/visits/create", {
      method: "POST",
      headers: {
    ...
    <div className="p-4 max-w-4xl mx-auto">
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-2 text-sm text-yellow-800">
          {error === "Patient not found" ? "No record found for this Patient ID or phone number." : error}
        </div>
      )}

      {!patient ? (
        <div className="bg-white rounded-lg p-6 border text-sm text-gray-600">
          Search for a valid Patient ID or phone number to view visit history.
        </div>
      ) : (
        <>

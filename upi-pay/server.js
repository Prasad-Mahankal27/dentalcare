...
const UPI_ID = typeof process.env.UPI_ID === "string" ? process.env.UPI_ID.trim() : "";
const UPI_NAME = typeof process.env.UPI_NAME === "string" ? process.env.UPI_NAME.trim() : "";
  ...
  try {
    ...
    if (!UPI_ID || !UPI_NAME) {
      return res.status(503).json({
        error: "Clinic UPI settings are not configured. Please update UPI ID and UPI name in Settings."
      });
    }
...
app.get("/status/__health__", (_req, res) => {
  res.json({ status: "ok" });
});

...
def send_document():
    if not ACCESS_TOKEN or not PHONE_NUMBER_ID:
        return jsonify({"message": "WhatsApp credentials missing"}), 500

    recipient = normalize_phone(request.form.get("to") or (request.get_json(silent=True) or {}).get("to"))
    if not recipient:
        return jsonify({"message": "Missing recipient phone in 'to'"}), 400

    uploaded = request.files.get("file")
    if not uploaded:
        return jsonify({"message": "Missing PDF file in 'file' form field"}), 400

    filename = (request.form.get("filename") or uploaded.filename or "report.pdf").strip() or "report.pdf"
    ...
    try:
        ...
        return jsonify({"message": "WhatsApp media upload failed", "error": str(exc)}), 502

    try:
        upload_body = upload_response.json()
    except ValueError:
        upload_body = {"message": "WhatsApp media upload returned non-JSON", "status": upload_response.status_code}

    if upload_response.status_code >= 400 or not upload_body.get("id"):
        app.logger.error("Media upload failed (status=%s): %s", upload_response.status_code, upload_body)
        return jsonify({"message": "WhatsApp media upload failed", "details": upload_body}), upload_response.status_code or 502

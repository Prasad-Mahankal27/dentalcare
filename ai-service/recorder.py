...
def _resolve_input_device() -> int | None:
    """Return a usable input device index, preferring the system default."""
    default_device = sd.default.device
    default_input = default_device[0] if isinstance(default_device, (list, tuple)) else default_device

    if isinstance(default_input, (int, np.integer)) and int(default_input) >= 0:
...
def start() -> None:
    ...
    selected_device = _resolve_input_device()
    ...
    try:
        ...
        print(f"[Recorder] Recording started using device: {device_info['name']}")
    except Exception as exc:
        _stream = None
        _is_recording = False
        raise RuntimeError(
            "Unable to access the microphone. Please allow microphone access for Orisyn and verify an input device is available."
        ) from exc
